...
We start with the definition where inputs $$Q, K, V$$ typically have shapes $$(B, \text{nhead}, N, nd)$$. Since batch and head dimensions are independent from attention weights computation, we focus on the sequence length $$N$$ and the embedding dimension $$nd$$.
...
$$
\begin{equation}
O[k, :]=\sum_{i=1}^N X_s[k, i]\cdot V[i, :]~,
\end{equation}
$$
where
$$
\begin{align}
X_s[k, i]&=\text{softmax}\{i=1:N\vert x_i=X[k,i]=\frac{Q[k, :]K[i, :]^T}{\sqrt{d_k}}\}\\
&=\{i=1:N|\frac{e^{x_i}}{\sum_{j=1}^N e^{x_j}}\}~.
\end{align}
$$
...
We cannot simply exponentiate the dot products because $$e^{x_i}$$ can easily overflow floating-point limits. To fix this, we use safe softmax. This requires finding the maximum value in the row ($$m_N$$) and subtracting it from every element before exponentiation.
...
This approach forces us to separate the calculation into multiple passes. We cannot compute the output until we have finished scanning the entire row to find $$m_N$$ and $$d_T$$. This requires repeated reading and writing of the $$N \times N$$ matrix makes standard attention memory-bounded.
...
Then we have two main steps. In loop 1: $$i=1 \rightarrow N$$ when moving from step $$i-1$$ to $$i$$, we introduce a new value $$x_i$$. If $$x_i > m_{i-1}$$, our maximum changes. We must rescale our previous accumulations to account for this change.
...
O'_i &= \sum_{j=1}^i \frac{e^{x_j - m_i}}{d'_i} \cdot V[j, :] \\ 
     ...
     & = \sum_{j=1}^{i-1} \frac{e^{x_j - m_{i-1}}}{d'_{i-1}} \cdot \left( \frac{d'_{i-1}}{e^{x_j - m_{i-1}}} \cdot \frac{e^{x_j - m_i}}{d'_i} \right) \cdot V[j, :] + \frac{e^{x_i - m_i}}{d'_i} \cdot V[i, :] \\
     & = O'_{i-1} \cdot \left( \frac{d'_{i-1}}{d'_i} \cdot e^{m_{i-1} - m_i} \right) + \frac{e^{x_i - m_i}}{d'_i} \cdot V[i, :]
\end{align}
$$

Upon reaching the N-th step with these recursive formulas, we have $$O'_T[k, :] = O[k, :]$$.
By doing this, we can compute the output in a single pass without needing the computation of $$X$$ matrix, and write/read it repeatedly to get global max and sum upfront. The next blog post will focus on how to implement this in an efficient way using CUDA.